...
void matrix::geneticAlgorithm(int populationSize, int generations, double stopTime, std::vector<int>& minCycle)
{
	...
	//size of this array will be:
	// -population size
	// -plus space for further cycles generated by crossing (50% of population size)
	// -plus space for cycles generated by mutation (25% of population size)
	// to sum up: whole array has 175% of base populationSize, first 100% for population, next 50% for crossing results, next 25% for mutation results

	//Population array is done this way due to prevention of reallocating memory
	//push_back() and shrink() would be easier but much longer for big populations
...
std::vector<int> matrix::mutationRandom(std::vector<int> parent, int levelsOfMutation)
{
	...
	{
		//rand elements
		int idx1 = rand() % parent.size();
		int idx2 = rand() % parent.size();

		while (idx1 == idx2)
		{
			idx2 = rand() % parent.size();
		}
